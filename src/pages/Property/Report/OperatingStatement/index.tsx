...
const CommunityAnnouncementIndex = () => {
  ...
  const businessItems = useMemo(
    () =>
      dictList
        .filter(item => item.tableDesc === 'business')
        .slice()
        .reverse(),
    [dictList]
  )
  ...
  useEffect(() => {
    if (businessItems.length) {
      setSelectedButton(businessItems[0]?.value || '0')
    }
  }, [businessItems])

  const selectedItem = useMemo(
    () => businessItems.find(item => item.value === selectedButton),
    [businessItems, selectedButton]
  )
