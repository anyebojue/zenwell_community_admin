...
import { QueryReportFeeDetailOwnerReply } from 'api/model/property/report/queryReportFeeDetailOwnerModel'
...
const TableData: React.FC<TableDataProps> = () => {
  const { page, list, exportUrl } = useSelector(
    (state: RootState) => state.QueryReportFeeDetailOwnerSlice
  )
  ...
  const columns = [
    { field: 'contractName', headerName: '合同', flex: 1 },
    {
      field: 'ownerName',
      headerName: '业主',
      flex: 1,
      renderCell: ({ row }: { row: QueryReportFeeDetailOwnerReply }) =>
  ...
  const rows = list.map((item: QueryReportFeeDetailOwnerReply, index: number) => {
  ...
  return (
    <>
      ...
      <DataGrid
        ...
        rows={rows as QueryReportFeeDetailOwnerReply[]}
        columns={columns}
