...
import { find, deleteByIds } from 'modules/property/parking/carBlackWhite'
...
const FeeConfigIndex = () => {
  ...
  const modifiedFloorList = [
    { id: '', name: '全部' },
    { id: '2222', name: '白名单' },
    { id: '1111', name: '黑名单' }
  ]
  const [dialogValue, setDialogValue] = useState<CarBlackWhiteReply | undefined>()
          ...
          <Box sx={contentBoxStyle}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Typography variant="h6">
                {modifiedFloorList.filter(item => item.id === selectedButton)[0]?.name}车辆
              </Typography>
              <Button
                size="small"
                variant="contained"
