...
import { find, deleteByIds } from 'modules/property/parking/machine'
...
const FeeConfigIndex = () => {
  ...
  const { list: parkingBoxList } = useSelector((state: RootState) => state.ParkingBoxSlice)

  const [selectedButton, setSelectedButton] = useState<string>('')
  const modifiedFloorList = [{ id: '', boxName: '全部' }, ...parkingBoxList]
  const [dialogValue, setDialogValue] = useState<MachineReply | undefined>()
        ...
        <Box sx={{ width: '90%', height: '100%' }}>
          ...
          <Box sx={contentBoxStyle}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Typography variant="h6">摄像头</Typography>
              <Button
                size="small"
                variant="contained"
                color="error"
