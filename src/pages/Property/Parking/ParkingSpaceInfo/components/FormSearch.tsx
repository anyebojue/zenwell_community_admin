...
import { ParkingSpaceInfoParams } from 'api/model/property/parking/parkingSpaceInfoModel'
...
const FormSearch: React.FC<SearchFormProps> = ({ selectedRows, setDelOpen }) => {
  ...
  const [searchParams, setSearchParams] = useState<ParkingSpaceInfoParams>({
    paId: '',
    num: '',
    stateCd: ''
  })
  ...
  const handleInputChange = useCallback(
    (field: keyof ParkingSpaceInfoParams) => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleReset = useCallback(() => {
    const initialParams = { paId: '', num: '', stateCd: '' }
      ...
      <Stack direction="row" spacing={3} component="form" sx={{ mt: 2, mb: 1.5 }}>
        <FormControl sx={{ width: { xs: '100%', md: '25ch' } }} variant="outlined">
          <TextField
            ...
            onChange={handleSelectChange('paId')}
            variant="outlined"
            sx={textFieldStyles}
          >
            {list.map(option => (
