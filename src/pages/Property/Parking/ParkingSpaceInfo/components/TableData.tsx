...
import { ParkingSpaceInfoReply } from 'api/model/property/parking/parkingSpaceInfoModel'
import { find } from 'modules/property/parking/parkingSpaceInfo'
...
interface TableDataProps {
  ...
  setDialogValue: Dispatch<SetStateAction<ParkingSpaceInfoReply | undefined>>
...
const TableData: React.FC<TableDataProps> = ({
  ...
  const handleActionClick = useCallback(
    (actionType: string, row: ParkingSpaceInfoReply) => {
      switch (actionType) {
        case 'edit':
          setDialogType('edit')
          setDialogValue(row)
  ...
  const renderActionButtons = (row: ParkingSpaceInfoReply) =>
    [
    ...
    <DataGrid
      ...
      columns={[
        ...
        },
        {
          ...
          headerName: '车位类型',
          flex: 1,
          renderCell: ({ row }) => <Chip label={statusTypeValue[row.parkingType!] || '未知类型'} />
        },
        { field: 'area', headerName: '面积', flex: 1 },
