...
import { deleteByIds, find } from 'modules/property/communitys/release'
...
const ReleasesIndex = () => {
  ...
  const [dialogValue, setDialogValue] = useState<ReleaseReply>()
  ...
  const getDeleteData = useCallback(() => {
    if (selectedRows.size > 0) {
      return list
        ...
        .map(item => ({ id: item.id!, applyPerson: item.applyPerson! }))
        .filter(item => item.id && item.applyPerson)
    }
    if (dialogValue) {
      return dialogValue.id && dialogValue.applyPerson
        ? [{ id: dialogValue.id, applyPerson: dialogValue.applyPerson }]
        : []
    }
    return []
  ...
  const deleteNames = deleteData.map(item => `${item.applyPerson}的物品`)
      ...
      <Box sx={{ width: '100%' }}>
        ...
        <Box sx={contentBoxStyle}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Typography variant="h6">物品放行</Typography>
            <Button
              size="small"
              variant="contained"
              color="error"
