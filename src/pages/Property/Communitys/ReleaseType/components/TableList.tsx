...
const TableList = ({
  ...
  rows: ReleaseTypeReply[]
  columns: Column<ReleaseTypeReply>[]
  setDialogValue: Dispatch<SetStateAction<ReleaseTypeReply | undefined>>
  ...
  const renderValue = (value: ReleaseTypeReply[keyof ReleaseTypeReply] | undefined) => {
            ...
            {paginatedRows.length > 0 ? (
              paginatedRows.map(row => (
                <TableRow onClick={() => setDialogValue(row)} key={row.id} sx={tableRowStyle}>
                  ...
                  {columns.map(column => {
                    const value = row[column.key as keyof ReleaseTypeReply]
                    return (
                      <TableCell
                        key={column.key}
                        align={column.align}
