...
const TableList = ({
  ...
  rows: SpectionItemReply[]
  columns: Column<SpectionItemReply>[]
  ...
  const renderValue = (value: SpectionItemReply[keyof SpectionItemReply] | undefined) => {
            ...
            {paginatedRows.length > 0 ? (
              paginatedRows.map(row => (
                <TableRow onClick={() => setDialogValue(row)} key={row.id} sx={tableRowStyle}>
                  ...
                  {columns.map(column => {
                    const value = row[column.key as keyof SpectionItemReply]
                    return (
                      <TableCell
                        key={column.key}
                        align={column.align}
