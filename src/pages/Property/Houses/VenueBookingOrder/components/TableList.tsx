...
const TableList = ({
  ...
  rows: SpacePersonReply[]
  columns: Column<SpacePersonReply>[]
  ...
  const renderValue = (value: SpacePersonReply[keyof SpacePersonReply] | undefined) => {
            ...
            {paginatedRows.length > 0 ? (
              paginatedRows.map(row => (
                <TableRow onClick={() => setDialogValue(row)} key={row.id} sx={tableRowStyle}>
                  {columns.map(column => {
                    const value = row[column.key as keyof SpacePersonReply]
                    return (
                      <TableCell
                        key={column.key}
                        align={column.align}
