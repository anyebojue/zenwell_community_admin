...
import { OwnerInvoiceParams } from 'api/model/property/houses/ownerInvoiceModel'
...
const FormSearch: React.FC<SearchFormProps> = ({ selectedRows, setDelOpen }) => {
  ...
  const [searchParams, setSearchParams] = useState<OwnerInvoiceParams>({
    ownerName: '',
    invoiceType: '',
    invoiceName: ''
  })
  ...
  const handleInputChange =
    (field: keyof OwnerInvoiceParams) => (event: ChangeEvent<HTMLInputElement>) => {
      ...
      <Stack direction="row" spacing={3} component="form" sx={{ mb: 1.5 }}>
        ...
        <FormControl sx={{ width: { xs: '100%', md: '25ch' } }} variant="outlined">
          <TextField
            ...
            onChange={handleInputChange('invoiceType')}
            variant="outlined"
            sx={textFieldStyles}
          >
            {[
        ...
        <FormControl sx={{ width: { xs: '100%', md: '25ch' } }} variant="outlined">
          <TextField
            ...
            variant="outlined"
            sx={textFieldStyles}
            value={searchParams.invoiceName}
            onChange={handleInputChange('invoiceName')}
          />
        </FormControl>
      </Stack>
