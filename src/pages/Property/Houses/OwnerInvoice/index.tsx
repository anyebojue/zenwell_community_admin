...
import { deleteByIds, find } from 'modules/property/houses/ownerInvoice'
...
const OwnerInvoicesIndex = () => {
  const dispatch = useDispatch<AppDispatch>()
  const { page, list } = useSelector((state: RootState) => state.OwnerInvoiceSlice)
  const [dialogValue, setDialogValue] = useState<OwnerInvoiceReply>()
  ...
  const getDeleteData = useCallback(() => {
    if (selectedRows.size > 0) {
      return list
        ...
        .map(item => ({ id: item.id!, invoiceName: item.invoiceName! }))
        .filter(item => item.id && item.invoiceName)
    }
    if (dialogValue) {
      return dialogValue.id && dialogValue.invoiceName
        ? [{ id: dialogValue.id, invoiceName: dialogValue.invoiceName }]
        : []
    }
    return []
  ...
  const deleteNames = deleteData.map(item => item.invoiceName)
...
export default memo(OwnerInvoicesIndex)
