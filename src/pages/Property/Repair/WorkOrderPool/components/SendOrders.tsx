...
const SendOrders: React.FC<SendOrdersProps> = ({ dialogValue, sendOpen, setSendOpen }) => {
  ...
  const { list } = useSelector((state: RootState) => state.RepairStaffSlice)
  const [loading, setLoading] = useState(false)
  const [formData, setFormData] = useState({ staffId: '', content: '' })
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const params = {
          ...
          updateStatusMsg: {
            ...formData,
            staff_id: list.filter(item => item.staffId === formData.staffId)[0]?.staffName
          }
        }
    ...
    [dialogValue?.id, dialogValue?.repairSettingId, dispatch, formData, list]
  )
