...
const TableList = ({
  ...
  rows: RepairStaffReply[]
  columns: Column<RepairStaffReply>[]
  ...
  const renderValue = (value: RepairStaffReply[keyof RepairStaffReply] | undefined) => {
            ...
            {paginatedRows.length > 0 ? (
              paginatedRows.map(row => (
                <TableRow onClick={() => setDialogValue(row)} key={row.id} sx={tableRowStyle}>
                  {columns.map(column => {
                    const value = row[column.key as keyof RepairStaffReply]
                    return (
                      <TableCell
                        key={column.key}
                        align={column.align}
