...
import { ResourceStoreTypeParams } from 'api/model/property/purchase/resourceStoreTypeModel'
...
const FormSearch: React.FC<SearchFormProps> = ({ value, selectedRows, setDelOpen }) => {
  const dispatch = useDispatch<AppDispatch>()
  const { page } = useSelector((state: RootState) => state.ResourceStoreTypeSlice)

  const [openDialog, setOpenDialog] = useState(false)
  const [searchParams, setSearchParams] = useState<ResourceStoreTypeParams>({
    name: ''
  })

  const handleInputChange = useCallback(
    (field: keyof ResourceStoreTypeParams) => (event: ChangeEvent<HTMLInputElement>) => {
    ...
    async (params: ResourceStoreTypeParams & PaginationParams) => {
      ...
      try {
        const res = await dispatch(
          find({ 'page.num': page.num, 'page.size': page.size, ...params, storeId: value?.id })
        )
  ...
  const handleReset = useCallback(() => {
    const initialParams = { name: '', id: '', storeId: value?.id }
