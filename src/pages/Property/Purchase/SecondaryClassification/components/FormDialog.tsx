...
import { create, find, update } from 'modules/property/purchase/resourceStoreType'
import {
  Box,
  CircularProgress,
  FormLabel,
...
interface FormDialogProps {
  ...
  dialogValue?: ResourceStoreTypeReply
  openDialog: boolean
  dialogType: string
...
const FormDialog: React.FC<FormDialogProps> = ({
  ...
  const [formData, setFormData] = useState<ResourceStoreTypeParams>(initialFormData)
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const params = { ...formData, communityId: community?.id, storeId: value?.id }
        const action =
        ...
        await dispatch(
          find({ 'page.num': page.num || '1', 'page.size': page.size, storeId: value?.id })
        )
