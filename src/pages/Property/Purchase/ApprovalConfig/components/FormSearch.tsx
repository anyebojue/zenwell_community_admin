...
import { WorkflowParams } from 'api/model/property/purchase/workflowModel'
...
const FormSearch: React.FC<SearchFormProps> = ({ selectedRows, setDelOpen }) => {
  ...
  const [searchParams, setSearchParams] = useState<WorkflowParams>({
    flowName: '',
    flowType: ''
  })

  const handleInputChange = useCallback(
    (field: keyof WorkflowParams) => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleReset = useCallback(() => {
    const initialParams = { flowName: '', flowType: '' }
      ...
      <Stack direction="row" spacing={3} component="form" sx={{ mt: 2, mb: 1.5 }}>
        <FormControl sx={{ width: { xs: '100%', md: '25ch' } }} variant="outlined">
          <TextField
            ...
            variant="outlined"
            sx={textFieldStyles}
            value={searchParams.flowName}
            onChange={handleInputChange('flowName')}
          />
        </FormControl>
        <FormControl sx={{ width: { xs: '100%', md: '25ch' } }} variant="outlined">
          <TextField
            ...
            onChange={handleSelectChange('flowType')}
            variant="outlined"
            sx={textFieldStyles}
          >
            {[
