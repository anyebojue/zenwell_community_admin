...
import { deleteByIds, find } from 'modules/property/purchase/workflow'
...
const ParkingLotManagementIndex = () => {
  ...
  const [dialogValue, setDialogValue] = useState<WorkflowReply | undefined>()
  ...
  const getDeleteData = useCallback(() => {
    return Array.from(selectedRows)
      ...
      .filter(item => item)
      .map(item => ({ id: item!.id!, flowName: item!.flowName! }))
  }, [selectedRows, list])

  const deleteData = getDeleteData()
  const deleteIds = deleteData.map(item => item.id)
  const deleteNames = deleteData.map(item => item.flowName)
    ...
    <Box sx={{ mt: 3.5, width: '100%' }}>
      ...
      <Box sx={contentBoxStyle}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Typography variant="h6">审批流程</Typography>
          <Button
            size="small"
            variant="contained"
            color="error"
