...
import { deleteByIds, find } from 'modules/platform/reportConfiguration/reportCustomComponentRel'
  ...
  const handleDelete = useCallback(
    async (ids: string[]) => {
      ...
      try {
        ...
        await dispatch(find({ 'page.num': page.num, 'page.size': page.size, customId: value.id }))
      } catch (err: unknown) {
      ...
      <Box sx={{ width: '100%' }}>
        <Box sx={contentBoxStyle}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Typography variant="h6">{value.title}组件</Typography>
          ...
          <TableData valueData={value} setSelectedRows={setSelectedRows} setDelOpen={setDelOpen} />
        </Box>
      </Box>
      <Copyright />
      <FormDialog valueData={value} openDialog={openDialog} setOpenDialog={setOpenDialog} />
      <DeleteModal
        loading={loading}
        delOpen={delOpen}
